...
async fn create_asset(
    ...
    let new_asset = repository.create_asset(
        request.name,
        request.unit_value
    ).await?;
    Ok(Json(new_asset))
...
async fn update_asset(
    ...
    match repository.update_asset(
        request.id, request.name, request.unit_value
    ).await? {
...
mod test {
    ...
    async fn test_update_asset(db: PgPool) {
        ...
        let Json(update_asset) = update_asset(
            Admin, db.into(), Json(request)
        ).await.expect("success");
